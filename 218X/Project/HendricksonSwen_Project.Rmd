...
beim_2013to2017_avgs <- bay_eng_int_map %>% 
  group_by(geometry) %>% 
  summarise(avg = energy_intensity / 5) %>% 
  st_transform(crs = 4326)
...
```{r 3.2 DATA-IN: EDUCATION}
education_2013 <-
  read_csv("data/education/ACSST5Y2013.S1501_data_with_overlays_2021-12-05T013816.csv") %>% 
  select(c(1, 13:28), -c(ends_with("M"))) %>%
  mutate(name = substr(GEO_ID, 10, 20)) %>% select(-GEO_ID) 

education_2013 <- education_2013[2:8058, ] %>% 
  transmute(
    'total' = as.numeric(S1501_C01_006E),
    '1' = as.numeric(S1501_C01_007E),
    '2' = as.numeric(S1501_C01_008E),
    '3' = as.numeric(S1501_C01_009E),
    '4' = as.numeric(S1501_C01_010E),
    '5' = as.numeric(S1501_C01_011E),
    '6' = as.numeric(S1501_C01_012E),
    '7' = as.numeric(S1501_C01_013E),
    'name' = name  
  ) %>% 
  transmute(
    'education_level' = 
...
```{r 5.1 ANALYSIS: EISF vs. EDUCATION}
edu_EISF_2013 <- beim_2013 %>%
  left_join(education_2013, by = "name") %>% 
  select(education_level, energy_intensity) %>% 
  st_transform(crs = 4326)
...
ggplot(
  ...
  geom_point() +
  ggtitle("Energy Intensity vs. Education Level") +
  xlab("Education Level (Scored 1-7)") + 
  ylab("Energy Intensity (Btu/SF)") +
  geom_smooth(method = "lm") 

# as education goes up, energy intensity per SF decreases

model <- lm(energy_intensity ~ education_level, data = edu_EISF_2013)
...
edu_ECPC_2013 <- ECPC %>%
  filter(year == 2013, !is.na(usage_percap)) %>% 
  left_join(education_2013, by = "name") %>% 
  select(education_level, usage_percap) 
...
ggplot(
  ...
  geom_point() +
  ggtitle("Energy Consumption Per Capita vs. Education Level") +
  xlab("Education Level (Scored 1-7)") + 
  ylab("Energy Consumption Per Capita (Btu/Person)") +
  geom_smooth(method = "lm") 
